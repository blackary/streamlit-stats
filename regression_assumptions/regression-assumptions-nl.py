...
"""
Dangers: Coefficients can be biased.  Standard errors could be unreliable (then so too would be
t-statistic and p-value).
"""
""" 
1. Linearity
    - The regression needs to be linear in the parameters. You can have x^2 terms to represent
    squared data, so the relationship is linear with y (and additive).
2. Constant Error Variance
    - If you map out the distances between points and black fitted line, the variance scales with
    the values of the data. We don't want Heteroskedasticity.
...
def single_regression(x,y):
    """
    Fits a linear regression to y using a single predictive 
    x variable as input.
    """
...
def multi_regression(x_mult,y):
    """
    Fits a linear regression to y using multiple x variables as input.
    """
...
def run_vif(x: pd.DataFrame):
    """
    Calculate the correlation and VIF for a set of x variables.
    """
