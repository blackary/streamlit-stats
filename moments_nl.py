"""
Moments dashboard experiment.
1st moment (mu'1): Mean: sum(xi)/n = average distance from 0.
2nd moment (crude): sum(xi^2)/n = average squared distance from 0.
    Greater spread/variance in distribution, greater u'2.
    Is dependent on the 1st moment (mean).
2nd moment (centered): (sum(xi-mu'1)^2)/n = average squared distance from the mean.
    Better, because it depends on spread/variance of dataset, and doesn't depend on mean.
    This is the variance.  Spread about the mean.
3rd moment (standardised): skewness.  Just the extra effect, removed the effect of variance.
    How much difference between one side of the mean and the other.
4th moment (standardised): kurtosis.  Kurtosis doesn't depend at all on skewness, so don't
    need to remove its effect.  How big the shoulders are/fall off.
Estimation for sample, instead of population, removes one degree of freedom. So, divide by
